...
class DoubanDialogProcessor(DataProcessor):
    """
    Douban classification
    label: [0,1,2]->[pos,neu,neg]
    """
    def get_train_examples(self, data_dir):
    ...
    def get_test_examples(self, data_dir):
        import data_handle
        sents = data_handle.get_douban_test_responses()
        examples = []
        for i in range(len(sents)):
            guid = "%s-%s" % ("douban-all", i)
            text_a = sents[i]
            label = 1
    ...
    def _create_examples(self, data_dir, set_type):
        import data_handle
        train_text, train_label, dev_text, dev_label = data_handle.read_douban_dialog()
        examples = []
        if set_type == "train":
...
glue_processors = {
    ...
    "rte": RteProcessor,
    "wnli": WnliProcessor,
    "douban_dialog": DoubanDialogProcessor,
}
