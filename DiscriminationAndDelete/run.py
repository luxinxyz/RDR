...
def metric(label, pred):
    ...
    cr = classification_report(label, pred, digits=4, target_names=["Pos", "Neutral", "Neg"])
...
def train(args, train_dataset, eval_dataset, model, tokenizer, global_step, tb_writer):
    """ Train the model """
    ...
    best_res = {"w_f1": 0.0, "m_f1": 0.0, "acc": 0.0, "step": 0}  # save the best f1 score
    for _ in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and epoch_step % args.logging_steps == 0:
                    logs = {}
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:
                        ...
                        for key, value in results.items():
                            ...
                            logs[eval_key] = value
                        if best_res["w_f1"] < results["w_f1"]:
                            best_res["w_f1"] = results["w_f1"]
                            best_res["m_f1"] = results["m_f1"]
                            best_res["acc"] = results["acc"]
                            best_res["step"] = epoch_step
                    ...
                    logger.info(json.dumps({**logs, **{'step': epoch_step}}))

                if args.local_rank in [-1, 0] and args.save_steps > 0 and epoch_step % args.save_steps == 0:
    ...
    return global_step, tr_loss / global_step, best_res
...
def evaluate(args, model, tokenizer, eval_dataset, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        result = metric(out_label_ids, preds)
...
def test(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        output_eval_file = os.path.join(eval_output_dir, "test_res.txt")
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, test=False):
    ...
    if test == True:
        ...
        name = 'dev' if evaluate else 'train'
    cached_features_file = os.path.join(args.data_dir, 'cached_{}_{}_{}_{}'.format(name, list(filter(None, args.model_name_or_path.split('/'))).pop(), str(args.max_seq_length), str(task)))
...
def main():
    ...
    # logging.info("device: %s" % args.device)   # logger error
    # Setup logging
    ...
    iter = 5
    res = {"w_f1": [], "m_f1": [], "acc": [], "step": []}
    global_step = 0
    for i in range(iter):
        logger.info("=" * 40 + "Iter " + str(i) + "=" * 40)
        ...
        if args.do_train:
            ...
            eval_dataset = load_and_cache_examples(args, args.task_name, tokenizer, evaluate=True)
            global_step, tr_loss, best_res = train(args, train_dataset, eval_dataset, model, tokenizer, global_step, tb_writer)
            logger.info(" global_step = %s, average loss = %s, best_res = %s", global_step, tr_loss, str(best_res))
            for k in res:
                res[k].append(best_res[k])

    logger.info("avg acc: %f" % (float(np.sum(res["acc"])) / len(res["acc"])))

    logger.info("avg w_f1: %f" % (float(np.sum(res["w_f1"])) / len(res["w_f1"])))

    logger.info("avg m_f1: %f" % (float(np.sum(res["m_f1"])) / len(res["m_f1"])))

    logger.info(" ".join(["{}: {}".format(key, str(["%.4f" % v for v in res[key]])) for key in res]))
